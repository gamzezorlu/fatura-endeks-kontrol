...
def assign_segment(avg_consumption, segment_thresholds):
    """Tüketim ortalamasına göre segment ve threshold belirle"""
    if pd.isna(avg_consumption) or avg_consumption == 0:
        return 'A', segment_thresholds['A']
    elif avg_consumption < 100:
        return 'A', segment_thresholds['A']
    elif avg_consumption < 1000:
        return 'B', segment_thresholds['B']
    elif avg_consumption < 5000:
        return 'C', segment_thresholds['C']
    elif avg_consumption < 10000:
        return 'D', segment_thresholds['D']
    elif avg_consumption < 50000:
        return 'E', segment_thresholds['E']
    elif avg_consumption < 100000:
        return 'F', segment_thresholds['F']
    else:
        return 'G', segment_thresholds['G']

def analyze_facility(df, tesisat_no, analysis_year, analysis_month, segment_thresholds):
...
if uploaded_file is not None:
    try:
        ...
        # Tarih parse et - güvenli şekilde
        df['yil'] = None
        df['ay'] = None
        ...
        st.markdown("#### 🎯 Segment Bazlı Anomali Eşikleri (%)")
        st.caption("Her segment için farklı threshold değeri belirleyebilirsiniz. Büyük tüketiciler için daha yüksek değerler önerilir.")
        
        col1, col2, col3, col4 = st.columns(4)
        
        with col1:
            threshold_a = st.number_input("Segment A (0-100 m³)", min_value=0.0, max_value=100.0, value=20.0, step=1.0, key='th_a')
            threshold_b = st.number_input("Segment B (100-1K m³)", min_value=0.0, max_value=100.0, value=25.0, step=1.0, key='th_b')
        
        with col2:
            threshold_c = st.number_input("Segment C (1K-5K m³)", min_value=0.0, max_value=100.0, value=30.0, step=1.0, key='th_c')
            threshold_d = st.number_input("Segment D (5K-10K m³)", min_value=0.0, max_value=100.0, value=35.0, step=1.0, key='th_d')
        
        with col3:
            threshold_e = st.number_input("Segment E (10K-50K m³)", min_value=0.0, max_value=100.0, value=40.0, step=1.0, key='th_e')
            threshold_f = st.number_input("Segment F (50K-100K m³)", min_value=0.0, max_value=100.0, value=45.0, step=1.0, key='th_f')
        
        with col4:
            threshold_g = st.number_input("Segment G (100K+ m³)", min_value=0.0, max_value=100.0, value=50.0, step=1.0, key='th_g')
        ...
        if st.button("🔍 Analizi Başlat", type="primary", use_container_width=True):
            with st.spinner('Analiz ediliyor...'):
                ...
                for idx, tesisat_no in enumerate(unique_tesisats):
                    result = analyze_facility(df, tesisat_no, analysis_year, analysis_month, segment_thresholds)
                    if result:
...
if 'results' in st.session_state:
    ...
    for r in results:
        ...
        if seg not in segment_stats:
            segment_stats[seg] = {'total': 0, 'anomaly': 0, 'threshold': r['segment_threshold']}
    ...
    if filtered_results:
        ...
        for r in filtered_results:
            export_data.append({
                ...
                'Segment': r['segment'],
                'Segment Threshold (%)': f"{r['segment_threshold']:.0f}",
    ...
    if total_pages > 0:
        ...
        for idx, result in enumerate(page_results, start=start_idx + 1):
            ...
            expander_title = (
                ...
                f"Segment: {result['segment']} (Eşik: {result['segment_threshold']:.0f}%) | "
            ...
            with st.expander(expander_title):
                ...
                with col1:
                    ...
                    st.write(f"Segment: **{result['segment']}** (Eşik: {result['segment_threshold']:.0f}%)")
